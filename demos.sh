...
cp /root/sbel/demos/viper_real_slope/blender_viper_render.py /root/sbel/outputs/FSI_Viper_RealSlope_SlopeAngle_15/
cp -r /root/sbel/demos/viper_real_slope/obj_for_render/ /root/sbel/outputs/FSI_Viper_RealSlope_SlopeAngle_15/
for i in {1..25}
do
        cd /root/sbel/outputs/FSI_Viper_RealSlope_SlopeAngle_15/ && blender --background --python ./blender_viper_render.py $i
done
cd /root/sbel/outputs/FSI_Curiosity_Uphill_1/ && ffmpeg -r 1 -f image2 -s 1920x1080 -i %d.png -vframes 25 -vcodec libx264 -crf 1 -pix_fmt yuv420p viper_animation.mp4
